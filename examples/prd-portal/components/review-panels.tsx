...
import type { ReviewIssue } from '@/lib/review-data';
...
export function ReviewNavigation({ toc }: { toc: TocItem[] }) {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver((entries) => { const visible = entries.find((entry) => entry.isIntersecting); if (visible) setActiveId(visible.target.id); }, { rootMargin: '-15% 0px -75% 0px' });
    headings.forEach((heading) => observer.observe(heading));
    return () => observer.disconnect();
  }, [toc]);
  return <aside className="review-navigation"><h2><List aria-hidden="true" />本节目录</h2><nav className="review-toc">{toc.map((item) => <a key={item.url} href={item.url} data-depth={item.depth} data-active={activeId === item.url.replace(/^#/, '') || undefined}>{item.title}</a>)}</nav></aside>;
}
