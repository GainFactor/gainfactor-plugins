...
□ Phase 0：基线收集与确认
  ...
  □ 0.2 宿主交互提问机制获取 PRD/HLD/Contract 路径
  □ 0.3 宿主交互提问机制确认 Guardrails
...
2. 使用宿主交互提问机制获取 PRD/HLD/Contract 路径（模板见 `references/askuser-templates.md`）
3. 使用宿主交互提问机制确认 Guardrails 是否存在
...
- [ ] 若 block 存在，执行 `python3 <gainfactor-pm-plugin-root>/scripts/trace_lint.py --format json <LLD 路径>`
...
| 基线不明 | 使用宿主交互提问机制确认（模板见 `references/askuser-templates.md`） |
