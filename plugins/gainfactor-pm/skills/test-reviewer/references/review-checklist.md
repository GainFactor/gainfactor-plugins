...
- [ ] 是否先执行了 `python3 <gainfactor-pm-plugin-root>/scripts/trace_lint.py --format json <Test Spec 路径>`？
- [ ] 是否先执行了 `python3 <gainfactor-pm-plugin-root>/scripts/trace_build_rtm.py --format json <PRD 路径> <Test Strategy 路径> <Test Spec 路径>`？
...
- 自动化建议是否与风险匹配
- 若 `GainFactor-PM Automation Handoff.status != not_planned`，其 scenario_groups / executor / split / relay / dependency / pipeline_required 是否完整
