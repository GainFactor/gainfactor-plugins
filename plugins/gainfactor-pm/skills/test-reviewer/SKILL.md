...
2. 使用当前宿主的文件枚举能力 扫描 PRD、API Contract、HLD、LLD、Test Strategy、Guardrails
3. 宿主交互提问机制确认评审模式：
...
- 若存在 `GainFactor-PM Automation Handoff` 且 `status != not_planned`，其 executor / split / dependency / relay / pipeline_required 是否足以支撑 `$case-writing`（缺失 → P1）
...
- **通过**：输出准出证书，并明确是：
  ...
  - 同时补充 downstream handoff：
    - 文档下游：`$runbook-writer`
    - 若 `GainFactor-PM Automation Handoff.status = ready | partial`：自动化下游 `$case-writing`

## 交互规范

- 模式不明确时，先使用宿主交互提问机制，再继续评审
- 无证据时不要假定“应该已经执行过”
