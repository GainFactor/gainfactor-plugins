...
|--------|------|
| 评审资产仅位于原型沙箱 | ✅/❌ |
| `prototypeId` 与 Manifest 一致 | ✅/❌ |
| schema 为 `gainfactor-pm-prototype-review/v1` | ✅/❌ |
| URL/DOM/runtime 出口内容一致 | ✅/❌/N/A（注明 hash-router 降级） |
| 新增、删除、刷新与跨页恢复通过 | ✅/❌ |
| 零外部网络上传 | ✅/❌ |
...
1. 团队走查原型，收集交互反馈
2. 基于反馈迭代原型（重新执行 `/gainfactor-pm:prototype-designer`）
3. 原型确认后，执行 `/gainfactor-pm:api-writer` 定义接口契约
4. 执行 `/gainfactor-pm:hld-writer` 开始技术设计
