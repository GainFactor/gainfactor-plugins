...
5. **基于证据，不猜测**：前端仓库的技术栈、组件库、设计规范必须通过扫描代码获得；找不到证据时必须用宿主交互提问机制确认
...
11. **原型必须可直接评审**：默认接入插件提供的评审模式，使用户可以在页面上记录结构化意见，并让 AI 从公开页面出口读取；不得要求用户先下载文件或复制浏览器私有存储
...
使用当前宿主的文件枚举能力扫描以下内容（**只收集路径，暂不读取**）：
...
|-----------|------|
| 3/3 | 直接通过，进入 0.2 |
| 2/3 或 monorepo 命中 | 使用宿主交互提问机制向用户确认前端代码位置，确认后通过 |
| 0-1/3 且无 monorepo 信号 | 使用宿主交互提问机制中止并引导 |
...
- 这不是前端仓库？（建议直接进入 /gainfactor-pm:api-writer）
```
...
使用宿主交互提问机制确认识别结果、原型放置目录、是否有 Storybook 或设计规范文档、原型是否对接已有路由系统。
...
**必须读取**：PRD（提取 `REQ-*`、业务实体、验收标准）和 User Journey（Journey Graph、步骤节点、跳转关系、异常处理）。若无 User Journey，提示用户先执行 `/gainfactor-pm:uc-interviewer`。
...
- **页面数 > 8 时**：强制触发 宿主交互提问机制，要求用户缩减范围或分批实现

使用宿主交互提问机制展示页面清单（含预算裁剪结果）、组件缺口、排除项，确认后继续。
...
├── review-mode/                  # 插件评审模式资产（JS/CSS，无外部依赖）
...
完整读取 `../../references/prototype-review-mode.md`，将
`../../assets/prototype-review-mode/` 中的 JS/CSS 复制到原型沙箱的 `review-mode/`，
并按参考文档配置唯一 `prototypeId`。如果仓库已有等价评审工具，只有在其同时提供
结构化 URL/DOM 出口、Markdown 汇总和本地隐私边界时才复用，禁止重复注入两套工具。

评审资产必须保持在原型沙箱内，不新增 npm 依赖，不修改生产组件。多页面原型的每个
可评审入口都必须加载同一份资产和同一个 `prototypeId`。
在 Prototype Manifest 和交付摘要中记录启用状态、`prototypeId`、资产路径和 schema。
...
3. 以上都不确定 → 宿主交互提问机制询问用户的类型检查命令，不要猜
...
同时按 `../../references/prototype-review-mode.md` 的验收清单验证：新增、删除、刷新、
跨页面恢复、AI 读取出口和 Markdown 展开均可用，且不会读取或上传真实业务数据。
