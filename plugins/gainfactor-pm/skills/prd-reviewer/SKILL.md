...
1. **读取 PRD 文档**
   ...
   - 执行命令：
     - `python3 <gainfactor-pm-plugin-root>/scripts/trace_lint.py --format json <PRD文件路径>`
   - 如需理解脚本输出和问题码，参考：
...
1. 创建独立临时目录，禁止在用户项目中复制 Mock 文档、创建 `.gainfactor/portal` 或覆盖现有前端工程。
2. 调用 `$document-publisher`，按其 `publishing/publish` 流程把当前 PRD 导入该显式临时 target；使用稳定 slug `prd`、type `PRD` 和 collection `产品文档`。不要在本 Skill 复制发布器的创建、构建或状态提升步骤。
3. 如果阶段三已经产生结构化问题，先按 `$document-publisher` 的 `publishing/review-findings` 契约生成并校验临时 `.review.json`，再在导入时挂载。运行时数据结构为：
...
4. 按 `$document-publisher` 的 `publishing/preview` 流程启动隔离门户并返回已验证 URL；不在本 Skill 自行安装依赖或拼接地址。
5. 页面内容只能来自本轮用户指定的 PRD。公共模板只包含布局、组件、样式和通用空状态，禁止向其中加入示例业务正文。
6. PRD 更新后继续使用相同临时 target 和 slug；不要手工维护生成的 `content/docs/<slug>.mdx`。
