...
2. **读取关联的 PRD 文档**（先问后判）
   ...
   >
   > **证据路径**：检查 PRD 元数据中的「状态」字段。如无状态字段，使用 `宿主交互提问机制` 询问用户确认。
   >
   > **处理路径**：
   > | 情况 | 严重度 | 处理 |
   > |------|--------|------|
...
- [ ] HLD 是否包含 `TRACEABILITY-METADATA` block？
  - 缺失 → **P1**（文档质量缺陷，继续后续审查）
- [ ] 若 block 存在，执行 `python3 <gainfactor-pm-plugin-root>/scripts/trace_lint.py --format json <HLD 路径>`
  - 存在 error → **P0 阻塞**（trace-lint blocking issue）
  - 存在 warning → **P1**
- [ ] 若 PRD 路径可用，执行 `python3 <gainfactor-pm-plugin-root>/scripts/trace_build_rtm.py --format json <PRD 路径> <HLD 路径>`
...
2. **1:N 场景识别**（PRD 拆分为多个 HLD）
   ...
   - 如果未标注且未引用，**必须使用宿主交互提问机制确认是否为 1:N 场景**：
     ```
...
- **宿主交互提问机制**：PRD 来源确认、风险特征确认、证据不足澄清必须询问
