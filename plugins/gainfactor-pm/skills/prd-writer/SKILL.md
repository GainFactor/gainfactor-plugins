...
2. **基于证据，不猜测**：所有关于项目现状、已有能力、业务流程的描述必须有文档/代码依据；找不到证据时必须使用宿主交互提问机制确认，**禁止凭空推测**
3. **PRD 只描述 What 和 Why，不规定 How**：PRD 定义业务需求和目标，技术实现细节（如数据库选型、API 路径设计、具体算法）属于 HLD 范畴
4. **关键问题必须确认，非关键问题直接给建议**：减少不必要的交互，提高效率
5. **强制交互确认关键问题**：优先使用宿主的结构化提问工具；工具不可用时，按宿主兼容规则使用简短普通文本提问
...
在了解项目上下文后，如果当前宿主提供联网搜索能力，则搜索业界对类似问题的解决方案，为 PRD 撰写提供参考；不可用时明确跳过，不要伪造搜索结果。
...
- **禁止**假设项目约定 — 找不到就用宿主交互提问机制确认

**关于交互**：
- 有结构化提问工具时不要绕过它；工具不可用时按宿主兼容规则退化为简短普通文本提问
- 不要一次问超过 6 个问题
- 不要问非关键问题
