...
```
□ Phase 0: 基线与上下文
  □ 0.1 使用当前宿主的文件枚举能力扫描 PRD/API/HLD/LLD/Test Strategy/Guardrails
  □ 0.2 宿主交互提问机制确认最新批准基线
  □ 0.3 读取上游文档与已有测试资产
  □ 0.4 输出「上下文收集报告」
...
1. 使用当前宿主的文件枚举能力 扫描：
   ...
   - Guardrails
   - 现有测试文档/自动化资产
2. 使用 `references/askuser-templates.md` 的模板 宿主交互提问机制确认最新批准基线
3. 提取：
   - 关键需求与验收标准
   - 接口/事件/错误契约
...
- 自动化建议
- 必需证据
- GainFactor-PM Automation Handoff 所需信息（若该 case 会进入 GainFactor-PM 落地）
...
- 非功能验证范围与方法
- 面向 `gainfactor-pm-bot` `$case-writing` 的 `GainFactor-PM Automation Handoff`
- 不纳入本轮的内容及理由
...
6. 按 `../../references/gainfactor-pm-automation-handoff-contract.md` 输出 `GainFactor-PM Automation Handoff`：
   - 即使当前不计划落到 GainFactor-PM，也要显式写 `status: not_planned`
   - 若计划落到 GainFactor-PM，至少给出 `scenario_groups`、`recommended_executor`、`platform_case_strategy`、`pipeline_required`
   - 若 `status: ready`，则 handoff 应足以让 `$case-writing` 直接开始工作
7. 使用 `references/test-package-template.md` 输出最终文档
8. 对已保存的文档执行：
   - `python3 <gainfactor-pm-plugin-root>/scripts/trace_lint.py --format json <Test Spec 路径>`
   - `python3 <gainfactor-pm-plugin-root>/scripts/trace_build_rtm.py --format json <PRD 路径> <Test Strategy 路径> <Test Spec 路径>`
...
3. 环境或依赖能力会直接影响用例设计
4. 回归范围或自动化优先级需要业务取舍
5. 是否计划把本包继续落到 GainFactor-PM 自动化，会影响 `GainFactor-PM Automation Handoff.status`
...
- 开发内建验证前置条件
- 回归与自动化建议
- `GainFactor-PM Automation Handoff`
- 假设、豁免、待确认项
...
- 若声明 `GainFactor-PM Automation Handoff.status = ready`，则可直接交给 `gainfactor-pm-bot` 的 `$case-writing`
...
- `../../references/gainfactor-pm-automation-handoff-contract.md`：Test Spec 到 `gainfactor-pm-bot` 的下游 handoff 契约
