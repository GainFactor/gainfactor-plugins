...
> 这个项目现在应该从哪个 gainfactor-pm skill 开始？
...
## 示例 9：Test Spec 已准出，且已具备 GainFactor-PM Automation Handoff

### 用户输入

> Test Spec 已经 approved 了，我下一步想把它落成 GainFactor-PM 自动化脚本，应该用哪个命令？

### 观察到的证据

- `test-spec.md` metadata 显示 `artifact.type: TEST_SPEC`，`status: approved`
- 同一文档中存在 `## GainFactor-PM Automation Handoff`
...
1. `$case-writing`：你当前目标是进入 GainFactor-PM 自动化落地，而且 approved Test Spec 已经包含 `GainFactor-PM Automation Handoff.status = ready`。这是最直接、最小返工的下一步。
...
- 用户问的是阶段 / artifact 对应的 canonical skill invocation
...
```markdown
User Journey 对应的 `gainfactor-pm` skill 是 `$uc-interviewer`。
