...
3. **基于证据，不猜测**：所有关于现有架构、技术栈、已有能力的描述必须有文档/代码依据；找不到证据时必须使用宿主交互提问机制确认，**禁止凭空推测**
...
10. **强制交互确认关键技术细节**：按宿主兼容规则选择结构化提问工具或简短普通文本提问
...
在阶段零完成后，如果识别到需要拆分，**必须使用宿主交互提问机制确认**：
...
使用当前宿主的文件枚举能力**广泛扫描**以下类型的文档，**只收集文件路径，暂不读取内容**：
...
**如果发现无依据的猜测性内容，必须删除或通过宿主交互提问机制确认。**
...
**校验（写入文件后执行）：**
```bash
python3 <gainfactor-pm-plugin-root>/scripts/trace_lint.py --format json <HLD 路径>
```
若存在 blocking issue（error），必须修正后再输出完成结论。若 PRD 路径可用，额外执行：
```bash
python3 <gainfactor-pm-plugin-root>/scripts/trace_build_rtm.py --format json <PRD 路径> <HLD 路径>
```
