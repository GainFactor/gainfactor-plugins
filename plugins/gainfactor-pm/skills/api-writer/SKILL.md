...
2. **有基线才产出**：没有 PRD 基线或边界确认时，必须使用宿主交互提问机制并停止产出。
...
```
□ 阶段 0：上下文收集
  □ 0.1 使用当前宿主的文件枚举能力 扫描 PRD/需求文档、已有 API 规范、现有服务说明
  □ 0.2 宿主交互提问机制确认要读取的文档与最新批准基线
  □ 0.3 执行 Guardrails trigger check

□ 阶段 1：边界/所有权确认
  □ 1.1 宿主交互提问机制确认服务/模块边界
  □ 1.2 确认数据所有权（source of truth）
  □ 1.3 确认主要消费者与调用方向
  □ 1.4 确认与既有接口/能力的关系

□ 阶段 2：合同类型选择
  □ 2.1 宿主交互提问机制确认 contract 类型（HTTP/GraphQL/gRPC/Event/...）
  □ 2.2 确认输出格式
