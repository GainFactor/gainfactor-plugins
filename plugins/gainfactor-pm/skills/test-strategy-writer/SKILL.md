...
```
□ Phase 0: 基线与上下文
  □ 0.1 使用当前宿主的文件枚举能力扫描 PRD/API/HLD/Guardrails/ADR
...
1. 使用当前宿主的文件枚举能力 扫描 PRD、API Contract、HLD、Guardrails、ADR、已有测试规范
2. 使用 `references/askuser-templates.md` 中的模板 宿主交互提问机制确认最新批准基线
3. 读取文档并提取：
   - 业务目标、关键用户旅程、验收标准
   - API/事件边界、兼容性要求、错误语义
...
5. 若范围或风险容忍度不清晰，必须使用宿主交互提问机制让用户确认
...
8. 对已保存的文档执行：
   - `python3 <gainfactor-pm-plugin-root>/scripts/trace_lint.py --format json <Test Strategy 路径>`
   - `python3 <gainfactor-pm-plugin-root>/scripts/trace_build_rtm.py --format json <PRD 路径> <Test Strategy 路径>`
