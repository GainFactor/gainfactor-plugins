...
- 若存在多份 BRD、多个版本、或缺少批准证据，先用宿主交互提问机制让用户确认哪一份是当前有效 baseline
...
**工作模式**：默认 `开放发现 → 结构化确认`。只有当 BRD 和已确认上下文足以支持高质量选项时，才使用宿主交互提问机制给候选项；否则先让用户用 1-3 句描述，再把描述整理成选项确认。
...
如果选择“跳转到已定义 Journey”，用宿主交互提问机制选择目标 Journey 与入口步骤。
...
```bash
python3 <gainfactor-pm-plugin-root>/scripts/trace_lint.py --format json --profile journey-profile-v1 [Journey路径]
```
