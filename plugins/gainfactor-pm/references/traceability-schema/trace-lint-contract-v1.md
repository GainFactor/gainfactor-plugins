...
`trace-lint` 是 `gainfactor-pm` traceability metadata 的基础校验器。  
...
```bash
python3 <gainfactor-pm-plugin-root>/scripts/trace_lint.py
```
...
```bash
python3 <gainfactor-pm-plugin-root>/scripts/trace_lint.py <path...>
```
...
```bash
python3 <gainfactor-pm-plugin-root>/scripts/trace_lint.py docs/PRD-checkout.md
```

输出机器可读 JSON：

```bash
python3 <gainfactor-pm-plugin-root>/scripts/trace_lint.py --format json docs/PRD-checkout.md
```

在 reviewer / CI 中启用严格模式：

```bash
python3 <gainfactor-pm-plugin-root>/scripts/trace_lint.py --strict docs/PRD-checkout.md
```
...
```bash
python3 <gainfactor-pm-plugin-root>/scripts/trace_lint.py \
  plugins/gainfactor-pm/references/traceability-schema/prd-profile-v1.example.yaml
```
...
- 是否存在顶层字段：
  ...
  - `relations`
  - `waivers`
- `schema.name` 是否等于 `gainfactor-pm-traceability`
- `schema.version` 是否存在
- `schema.profile` 是否存在
